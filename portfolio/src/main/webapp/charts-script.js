...
google.charts.load('current', {'packages': ['geochart'], 'mapsApiKey': mapKey});
google.charts.setOnLoadCallback(drawChart);

/** Creates a chart and adds it to the page. */
function drawChart() {
  fetch(CHART_URL).then(response => response.json()).then((countryMap) => {
    ...
    const chart =
        new google.visualization.GeoChart(document.getElementById(CHART_ID));
    chart.draw(data, {});
  });
}